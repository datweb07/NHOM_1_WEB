        ...
        <div class="col-md-7">
            ...
            <?php else: ?>
                <div class="alert alert-warning" role="alert">
                    ...
                    Sản phẩm này hiện không có phiên bản khả dụng
                </div>
            <?php endif; ?>
...
<script>
    ...
    document.querySelectorAll('.variant-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            console.log('Phiên bản được chọn:', selectedVariantId);
        });
    });
    ...
    // Xử lý form thêm giỏ hàng
    document.querySelector('form[action="/gio-hang/them"]')?.addEventListener('submit', function(e) {
        const phienBanId = document.getElementById('selected-variant')?.value;
        const soLuong = document.getElementById('qty-input')?.value;
        const submitBtn = this.querySelector('button[type="submit"]');

        console.log('Submit form - phienBanId:', phienBanId, 'soLuong:', soLuong);

        // Validation phiên bản - Phải có giá trị số hợp lệ
        if (!phienBanId || phienBanId === '' || !parseInt(phienBanId)) {
            e.preventDefault();
            alert('Vui lòng chọn phiên bản sản phẩm');
            console.error('Validation failed - phienBanId:', phienBanId);
            return false;
        }
        ...
        if (submitBtn) {
            ...
            submitBtn.innerHTML = '<i class="fa fa-spinner fa-spin me-1"></i>Đang xử lý...';
        }
    });
