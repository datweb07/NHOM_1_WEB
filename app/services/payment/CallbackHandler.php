...
require_once __DIR__ . '/MomoGateway.php';
...
class CallbackHandler
{
    ...
    public function handleMomoCallback(array $data): array
    {
        ...
        if (!$isValidSignature) {
            $this->logSecurityViolation($transactionId, 'MOMO', 'SIGNATURE_FAILED', [
                'source_ip' => $sourceIp,
                'data' => $data
            ]);
        ...
        if ($elapsedTime > 3) {
            error_log("Momo callback processing took {$elapsedTime} seconds");
        }
